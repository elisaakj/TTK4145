...
class Resource(T)
{
    ...
    {
        mtx.wait();                    // låser muteksen
        
        if(busy){
            numWaiting[priority]++;   // øker antall ventende for prioriteten
			mtx.notify();             // frigir muteksen mens man venter
            sems[priority].wait();     // venter til ressursen blir ledig
            numWaiting[priority]--;   // reduserer antall ventende etter man har blitt vekket
        }
        
        busy = true;
        mtx.notify();                  // frigjør muteksen
    ...
    void deallocate(T v)
    {
        ...
        if(numWaiting[1] > 0){        // if(GetValue(PS[1]) < 0) HØY PRIORITETSKØ
            sems[1].notify();
        } else if(numWaiting[0] > 0){ // else if(GetValue(PS[0]) < 0) LAV PRIORITETSKØ
            sems[0].notify();
        } else {                      // INGEN VENTER
            mtx.notify();              
        }
